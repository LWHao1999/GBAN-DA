...
class DTIDataset(data.Dataset):
    def __init__(self, list_IDs, df, max_drug_nodes=290, protein_max_length=1000):
    ...
    def __getitem__(self, index):
        ...
        origin_v_p = v_p
        encoded_v_p = integer_label_protein(v_p, max_length=self.protein_max_length)


        protein_len = len(v_p)
        protein_mask = np.zeros(self.protein_max_length, dtype=np.float32)
        if protein_len <= self.protein_max_length:
            protein_mask[:protein_len] = 1.0
        else:
            protein_mask[:self.protein_max_length] = 1.0


        y = self.df.iloc[index]["Y"]

        return v_d, origin_v_p, torch.from_numpy(encoded_v_p), torch.from_numpy(protein_mask), torch.tensor(y)
