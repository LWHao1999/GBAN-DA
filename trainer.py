...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for i, (v_d, origin_v_p, v_p, protein_mask, labels) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            v_d, v_p, protein_mask, labels = v_d.to(device), v_p.to(device), protein_mask.to(device), labels.float().to(device)
    ...
    def train_da_epoch(self):
        ...
        for i, (batch_s, batch_t) in enumerate(tqdm(self.train_dataloader)):
            ...
            v_d, origin_v_p, v_p, protein_mask, labels = batch_s[0].to(device), batch_s[1], batch_s[2].to(device), batch_s[3].to(device), batch_s[4].float().to(device)

            v_d_t, origin_v_p_t, v_p_t, protein_mask_t, labels_t = (
                batch_t[0].to(self.device),
                batch_t[1],  # 原始蛋白质序列（字符串列表，无需转移设备）
                batch_t[2].to(self.device),
                batch_t[3].to(self.device),
                batch_t[4].float().to(self.device)
            )
            ...
            if self.current_epoch >= self.da_init_epoch:
                v_d_t, v_p_t, f_t, t_score = self.model(v_d_t, v_p_t, protein_mask_t)
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            self.model.eval()
            for i, (v_d, origin_v_p, v_p, protein_mask, labels) in enumerate(data_loader):
                ...
                if dataloader == "val":
                    ...
                    v_d, v_p, f, score = self.best_model(v_d, v_p, protein_mask)
                if self.n_class == 1:
                    n, loss = binary_cross_entropy(score, labels)
                else:
                    n, loss = cross_entropy_logits(score, labels)
